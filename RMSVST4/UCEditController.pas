...
type CEditController = class(TAggregatedObject,IEditController,IPluginBase)
      FPlugView:CPlugView;
...
begin
  ...
  WriteStream(state,STREAMMAGIC_CONTROLLER,IVST3.GetState);
  result:=kResultOk;
end;
...
begin
  ...
  IVST3.SetState(ReadStream(state,STREAMMAGIC_CONTROLLER));
  result:=kResultOk;
end;
...
begin
  ...
  // lets see if context is a IHost:
  context.QueryInterface(UID_IHostApplication,obj);
  if obj<>NIL then WriteLog('It s a Host!');
  IVST3.Initialize2(context);     // not called in reaper ??
  result:=kResultOk;
end;
