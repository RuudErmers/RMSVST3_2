...
begin
  ...
  classId:=TUID(IVST3.GetPluginInfo.PluginDef.vst3controllerid);
  result:=kResultOk;
end;
...
// In reaper,and with a combined Component, EditController it is not needed to Getstate/SetState
  WriteStream(state,STREAMMAGIC_PROCESSOR,IVST3.GetState);
  result:=kResultOk;
end;
...
// In reaper,and with a combined Component, EditController is not needed to Getstate/SetState
  IVST3.SetState(ReadStream(state,STREAMMAGIC_PROCESSOR));
  result:=kResultTrue;
end;
